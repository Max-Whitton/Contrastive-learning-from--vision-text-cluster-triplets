...
VIT_DINO = True  # boolean flag to use DINO vision transformer model
...
class TextEncoder(nn.Module):
    ...
    def __init__(self, vocab, image_feature_map_dim, args, nhot=False, loadsmallerfirst=False):
        super().__init__()
        self.args = vars(args) if args is not None else {}
        ...
        self.loadsmallerfirst = self.args.get("loadsmallerfirst", False)
    ...
    def add_to_argparse(parser):
        ...
        parser.add_argument("--loadsmallerfirst", action="store_true",
                            help="whether to load smaller models first")
    ...
    def load_state_dict(self, state_dict, strict=True):
        """
        Custom load_state_dict that supports loading smaller embedding matrices
        into a larger one by copying existing rows and initializing the rest.
        """

        print("was able to call the load_state_dict overwrite")

        if "embedding.weight" in state_dict:
            loaded_weight = state_dict["embedding.weight"]
            current_weight = self.embedding.weight

            loaded_vocab, emb_dim = loaded_weight.shape
            current_vocab, current_dim = current_weight.shape

            if emb_dim != current_dim:
                raise ValueError(
                    f"Embedding dim mismatch: checkpoint={emb_dim}, model={current_dim}"
                )

            if loaded_vocab < current_vocab:
                print(f"Expanding embedding: {loaded_vocab} → {current_vocab}")

                # Copy into current weights
                with torch.no_grad():
                    self.embedding.weight[:loaded_vocab] = loaded_weight

                    # Initialize remaining rows
                    nn.init.normal_(self.embedding.weight[loaded_vocab:])

                # Remove from state_dict so parent doesn't overwrite
                del state_dict["embedding.weight"]

            elif loaded_vocab == current_vocab:
                # Perfect match → let default loading handle it
                pass

            else:
                raise ValueError(
                    f"Checkpoint vocab ({loaded_vocab}) larger than model ({current_vocab})"
                )

        return super().load_state_dict(state_dict, strict)


    def expand_embedding(self, new_vocab_size=None, init="normal"):
        """
        Expand embedding matrix to match full vocab size (or a provided size),
        preserving existing learned weights.

        Args:
            new_vocab_size (int, optional): target vocab size. Defaults to full vocab.
            init (str): initialization for new rows ("normal", "xavier", "zeros", "mean")
        """
        ...
        # Copy old weights + init new ones
        with torch.no_grad():
            new_embedding.weight[:old_vocab_size] = old_weight

            if init == "normal":
                nn.init.normal_(new_embedding.weight[old_vocab_size:])
            elif init == "xavier":
                nn.init.xavier_uniform_(new_embedding.weight[old_vocab_size:])
            elif init == "zeros":
                nn.init.zeros_(new_embedding.weight[old_vocab_size:])
            elif init == "mean":
                mean_vec = old_weight.mean(dim=0, keepdim=True)
                new_embedding.weight[old_vocab_size:] = mean_vec
            else:
                raise ValueError(f"Unknown init: {init}")

        # 🔥 Replace embedding in-place
        self.embedding = new_embedding

        print(f"Expanded embedding: {old_vocab_size} → {new_vocab_size}")
...
class MultiModalModel(nn.Module):
    ...
    def forward(self, image, text, text_length, touch=False, return_image_features=False, return_text_outputs=False):
        # encode image and text
        image_features, image_feature_map = self.encode_image(image)
        if not touch:
            text_features, text_outputs = self.encode_text(text, text_length)
        else:
            text_features, text_outputs = self.touch_encoder(text, text_length)
...
class TripletModel(MultiModalModel):
    """
    Triplet model for multimodal learning
    """
    ...
    def encode_text(self, text, text_length, touch=False):
        if not touch:
            text_features, text_outputs, attns = self.text_embed(text, text_length)
        else:
            text_features, text_outputs, attns = self.touch_encoder(text, text_length)
        if self.normalize_features:
            # normalize text features
            text_features = F.normalize(text_features, p=2, dim=-1)
        return text_features, text_outputs
    ...
    def forward(self, image, text, text_length, touch=False, return_image_features=False, return_text_outputs=False):
        """Forward pass for triplet model. 
        
        Args:
            image: Image tensor
            text: Text tensor
            text_length: Text length tensor
            touch: If True, use touch encoder; else use audio encoder
            return_image_features: Whether to return image features and feature map
            return_text_outputs: Whether to return text outputs
            
        Returns:
            Tuple of (logits_per_image, logits_per_text, [image_features, image_feature_map], [text_outputs])
        """
        # encode image and text (using touch encoder if specified)
        image_features, image_feature_map = self.encode_image(image)
        if touch:
            text_features, text_outputs = self.touch_encoder(text, text_length)
            if self.normalize_features:
...
# class TouchClassifierModel(MultiModalModel):
#     """
#     Triplet model for multimodal learning
#     """

#     def __init__(self, vision_encoder, audio_encoder, touch_classes=1024, use_audio=False, args):
#         super().__init__(vision_encoder, audio_encoder, args)
#         if not use_audio:
#             self.touch_projection = nn.Linear(self.vision_encoder.last_cnn_out_dim, touch_classes)
#         else:
...
#             ret = ret + (text_outputs,)
#         if touch:
#             logits = self.touch_projection(image_features)
#             pred = 
